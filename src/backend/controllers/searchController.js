...
const searchHandler = expressAsyncHandler(async (req, res) => {
  ...
  const schema = z.object({
    ...
    include_categories: z
      .string()
      .regex(regex, "Must be a string separated by commas")
      .optional()
      .default("")
  ...
  if (!parsed.success) {
    res.status(400);
    throw new Error(
      `${parsed.error.issues[0].path}: ${parsed.error.issues[0].message}`,
    );
  }

  // Pipeline for authors.
  const authorPipeline = {
    ...(parsed.data.include_authors.length > 0 && {
      $in: parsed.data.include_authors,
    }),
    ...(parsed.data.exclude_authors.length > 0 && {
      $nin: parsed.data.exclude_authors,
    }),
  };

  // Pipeline for categories
  const categoryPipeline = {
    ...(parsed.data.include_categories.length > 0 && {
      $in: parsed.data.include_categories,
    }),
    ...(parsed.data.exclude_categories.length > 0 && {
      $nin: parsed.data.exclude_categories,
    }),
  };
  ...
  if (req.user) {
    ...
    if (blacklist.categories.length > 0)
      userPipeline.categories = { $nin: blacklist.categories };
    if (blacklist.authors.length > 0)
      userPipeline.authors = { $nin: blacklist.authors };
  }

  const results = await Manga.aggregate()
    .match(userPipeline)
    .match(
      Object.keys(authorPipeline).length > 0 ? { authors: authorPipeline } : {},
    )
    ...
    .match(
      Object.keys(categoryPipeline).length > 0
        ? { "categories.name": categoryPipeline }
        : {},
    )
    .facet({
